...
from ..models import Question, Answer, User, question_voter, VisitCount
...
def ds_list():
    ...
    return render_template('question/question_list.html', question_list=question_list, page=page,
                           kw=kw, so=so, visit_count=visit_count.count, view_name='question.ds_list')
...
def d_list():
    ...
    if so == 'recommend':
        sub_query = db.session.query(question_voter.c.question_id, func.count('*').label('num_voter')) \
            .group_by(question_voter.c.question_id).subquery()
        question_list = Question.query.filter(Question.category == 'Development') \
            .outerjoin(sub_query, Question.id == sub_query.c.question_id) \
            .order_by(sub_query.c.num_voter.desc(), Question.create_date.desc())
    elif so == 'popular':
        sub_query = db.session.query(Answer.question_id, func.count('*').label('num_answer')) \
    ...
    # 페이징
    question_list = question_list.paginate(page=page, per_page=10, error_out=False)
    return render_template('question/development_list.html', question_list=question_list, page=page, kw=kw, so=so,
                           visit_count=visit_count.count, view_name='question.d_list')
...
def cs_list():
    ...
    return render_template('question/question_list.html', question_list=question_list, page=page, kw=kw, so=so,
                           visit_count=visit_count.count, view_name='question.cs_list')
...
def ai_list():
    ...
    question_list = question_list.paginate(page=page, per_page=10, error_out=False)
    return render_template('question/question_list.html', question_list=question_list, page=page,
                           kw=kw, so=so, visit_count=visit_count.count, view_name='question.ai_list')


@bp.route('/recommend_list/')
def r_list():
    # 입력 파라미터
    page = request.args.get('page', type=int, default=1)
    kw = request.args.get('kw', type=str, default='')
    ...
    question_list = question_list.paginate(page=page, per_page=10, error_out=False)
    return render_template('question/question_list.html', question_list=question_list, page=page,
                           kw=kw, so=so, visit_count=visit_count.count, view_name='question.r_list')
...
def c_list():
    ...
    question_list = question_list.paginate(page=page, per_page=10, error_out=False)
    return render_template('question/question_list.html', question_list=question_list, page=page,
                           kw=kw, so=so, visit_count=visit_count.count, view_name='question.c_list')
